...
- `src/shared/schemas.ts`: Added `Omitted` status & `GenerationAttempt` type.
- `src/shared/ipc-channels.ts`: Added IPC channels.
- `src/preload/index.ts`: Exposed new API methods.
- `src/renderer/pages/Library.tsx`: Implemented version mutation & integration.
- `src/renderer/components/IdeaDetail.tsx`: Implemented UI for versions, zoom, and logic.
- `src/renderer/components/IdeaDetail.module.css`: Added styles for new UI elements.
...
### Verified Outputs (Detailed Mode)

The "Detailed" level successfully shifts to "Pattern-Based" complexity (snowflakes, scarf patterns) rather than "Tiny Lines," aligning with the "Mindful/Dopamine Decor" aesthetic.

![Baby Penguin with a Scarf](/Users/heymarcell/.gemini/antigravity/brain/0422553d-af39-4f13-889f-48cce563831d/uploaded_image_1767222265339.png)
```
